 ...
 */
function wait(delayInMilliseconds: any) {
...
async function fetchProperties({ address, propertyType }) {
  ...
  const propertiesFilteredByAddress = propertiesFilteredByType.filter((property: any) =>
 ...
 */
async function fetchPropertyDetails(propertyId: any) {
  if (Math.random() > 0.99) {
    throw new Error('An unexpected error occurred');
  }

  await wait(randomInteger(500));
  const propertyToReturn = sampleProperties.find((property: any) => property.id === propertyId);
